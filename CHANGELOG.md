...
### Security
- Ineligible / quarantined cached evidence now drops the entire cache-ref-derived envelope from model-facing output. `evidence show --json`, MCP `show_evidence_bundle`, `evidence list`, and materialize (including when `item_ids` is omitted) expose none of the attacker-writable cache-ref fields — named or unknown, at any length. Eligible items keep their live reconstruction. A byte bound is not the injection control. Residual of #1031.
