...
### Added
- `brigade evidence controls` emits a versioned, machine-readable control crosswalk (`brigade.control_crosswalk.v1` / `brigade.evidence_controls.v1`) mapping Brigade evidence claims to ISO/IEC 42001:2023, NIST AI RMF 1.0, NIST AI 600-1, NIST SP 800-218/SSDF, NIST SP 800-53 Rev. 5.1, EU AI Act, ISO/IEC 27001:2022, AICPA SOC 2, OWASP Agentic Top 10, and CSA AICM identifiers. It computes an evidentiary state per row (`evidenced_passed`, `evidenced_failed`, `untested`, `not_applicable`) from configured workspace artifacts and can regenerate `docs/control-crosswalk.md`. The header states that this is an evidence index and design documentation, not evidence that a control operated, and the template does not reproduce licensed ISO or AICPA clause text. Fixes #1406.
