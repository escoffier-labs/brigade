...
- Oversized-record completion salvage now uses a bounded structural JSON scanner instead of a brace-window regex scan. It validates exact paths only (top-level `method == "turn/completed"`, `params.threadId`, `params.turn.id`, `params.turn.status`), rejects duplicate keys and records cut before those paths complete, ignores `"turn/completed"` markers appearing inside unrelated string values, is immune to braces inside strings, never reads past the 64 KiB walk window, and never materializes the oversized text. A decoy nested `turn` object or stray `threadId` can no longer record a completion for another turn, and duplicate `status` keys (completed then failed) no longer yield a false success (#1200).
- App-server `_observed_completions` now holds one active completion entry per thread (bounded memory): `reset_capture` clears it before each turn so a reused turn id cannot inherit a stale completion, and building the over-cap result consumes the entry so the same id cannot satisfy a later result either (#1200).
- Over-cap codex app-server salvage is now reachable through the real stream and correctly gated on turn status. Bounded ingestion records the turn id and status of every `turn/completed` notification it sees — including oversized records, where only the metadata (never the text) is preserved — so a turn that genuinely completed while the capture cap was hit still yields `completed_observed=True` instead of being silently dropped before routing. The salvage signal additionally requires `status == "completed"`: an observed `turn/completed` with status `failed` or `interrupted` no longer reaches run transport as a successful worker result or satisfies downstream DAG prerequisites (#1200).
