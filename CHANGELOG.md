...
### Fixed
- User-scope Claude work-loop hooks can pin `hook-run --target` to a wired workspace, never treat the home directory as a work root, and latch off for the session after two consecutive timeouts instead of appending a doctor banner on every tool call. Unpinned events from an unwired git checkout use the same timeout isolation, so a stalled path resolve or `.git` check returns the timeout envelope instead of blocking the session. Project-scope `hooks install` refuses a home-directory target (that path is Claude Code user settings), and `hooks status` reports duplicate managed handlers and a widened user-settings path. Fixes #1051.
