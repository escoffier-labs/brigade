...
### Security
- Model-facing evidence payloads now run the eligibility sanitizer as the last transform on every exit (`materializeEvidenceBundle` both branches, `evidence list` / `listEvidenceBundles`, and `evidence show` / MCP `show_evidence_bundle`). Cache-sourced fields — including `id` and `resource_uri` taken from a cache filename — are wiped and then dropped or 24-hex-allowlisted only after that decision, so a pre-gate assignment cannot survive for an ineligible or quarantined item. Residual of #1031.
- Ineligible evidence-bundle aggregations no longer echo raw `sources.kind` as a `grouped_by_source` key. Facet buckets use the same allowlist-redacted `source_kind` the item projection emits, and `searchResultsByIDs` now runs `applySearchIntegrity` so the `item_ids` materialize path matches `search()`. Source-controlled `items.external_id`, `collections.external_id`, `actors.external_id`, and `raw_ref.path` are dropped for ineligible hits; only the opaque internal item `id` remains. Residual of #1032.
- Every model-facing evidence projection — `materializeEvidenceBundle` (both the `item_ids` and no-`item_ids` branches), `evidence show` / MCP `show_evidence_bundle`, and `listEvidenceBundles` — writes cache-ref-derived fields through one sanitizer. For ineligible / quarantined items that sanitizer drops or allowlist-validates every attacker-writable field recursively, including `query` on `evidence list` and `filters.project` / `filters.from` / `filters.to` on the no-`item_ids` show path. A byte bound is not the injection control: plants under 256 bytes must be dropped, not truncated. Eligible items keep their live reconstruction. Residual of #1031.
- Ineligible evidence projections now drop free-form `source_kind`, `collection_kind`, and `kind` unless they match the closed allowlist (adapter `Validate` only requires those fields non-empty). Cached `evidence show` / MCP `show_evidence_bundle` reconstruct `resource_uri` from the path-validated bundle id and do not echo attacker-writable cache-ref `query` or `resource_uri` into the model-facing payload. Completes #1031 and #1032.
- Evidence-ledger search, bundles, and cached `evidence show` / MCP `show_evidence_bundle` close three HIGH facade bypasses. Bundle queries now select artifact `metadata_json` whenever text is emitted, ingest stores separate URL and text digests, and a URL hash can no longer satisfy artifact-text integrity. The evidence cache stores item references only and regenerates bodies through the live eligibility and integrity gate. Ineligible search and bundle hits keep the opaque internal item `id` and closed-set fields and drop free-form `actor.name` / `actor.type` / `collection.name` plus source-controlled `items.external_id` / `collections.external_id` / `actors.external_id` / `raw_ref.path`. Fixes #1030, #1031, #1032.
