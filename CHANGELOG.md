...
### Changed
- Fleet claims fail closed on lost ownership (#1152). When the mid-run heartbeat learns another owner holds the claim (a renew answered 409 held-by-another), `repo_claim` no longer logs and lets the run continue unarbitrated: by default it logs one WARNING and aborts the guarded run (with no callback, via the same main-thread interrupt path as Ctrl-C), so two machines can never keep working the same repo after a supersede or expiry. Pass `on_claim_lost` to react differently, or set `BRIGADE_FLEET_CLAIM_LOSS=continue` (or `claim_loss_policy="continue"`) as the documented opt-out for solo machines, which restores the previous log-and-continue behavior. Fixes #1152.
- Fleet hub traffic now bypasses HTTP proxies (#1154): the fleet client's opener is built with an empty `ProxyHandler`, so `HTTP_PROXY`/`HTTPS_PROXY` can neither intercept the plain-HTTP bearer token en route to a tailnet hub nor blackhole its requests. The local event spool is hardened in the same pass: the spool directory is created 0700 and spool, lock, and temp files 0600 regardless of umask, opens are no-follow (`O_NOFOLLOW` where available) and refuse non-regular files such as FIFOs or symlinked spool paths, and atomic rewrites use exclusive, unpredictable temp names in the spool directory instead of the predictable `.tmp` path. Mode assertions are POSIX-only in tests. Fixes #1154.
- Fleet claim orphan cleanup closes its last leak windows (#1157). The background release scheduled after a lost acquire response now attempts immediately before backing off, so a `brigade run` that finishes within one backoff interval still frees the row instead of leaving it for the full TTL; and at exit, when an abandoned heartbeat re-acquire may still commit, the inline release retry also runs after a definitive `missing` answer — not only on `hub-unavailable` — because the straggler POST can land right after the exit release was told the claim was gone. Exhausted exit retries now log a WARNING naming the residual TTL exposure instead of tearing down silently.
