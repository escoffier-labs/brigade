...
### Security
- Opt-in `authority_store.isolation = "external-key"` HMAC-signs and verifies directory-authority bindings with a 0600 key stored outside the workspace and scanner-reachable tree (`$XDG_CONFIG_HOME/brigade/authority/store-hmac.key`, or `BRIGADE_AUTHORITY_KEY_FILE`). The flag is load-bearing on the write and verify paths: off leaves stores unsigned and `brigade doctor` WARNs about the residual same-uid write class. A `BRIGADE_AUTHORITY_KEY_FILE` override inside the resolved workspace is refused before the key is created. Fixes #957.
