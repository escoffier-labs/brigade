...
### Fixed
- `brigade work scanners init` and `operator quickstart` no longer fail on Windows when the released-runs-root binder cannot use POSIX `openat`. A missing `.brigade/scanners/runs` is a no-op; the walk uses the ledger's descriptor-relative helpers so Windows no-follow dirfd can still bind a clean unbound root. (#1036)
