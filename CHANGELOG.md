...
### Fixed
- Evidence crawl import (`miseledger import sourceharvest` / `crawl files|gitlog|docs`) and provenance backfill retry the SQLITE_BUSY family (primary code 5, including snapshot 517 and recovery 261) from a concurrent writer instead of failing the runbook. `busy_timeout` is 1s so retries compose to a few seconds, never minutes; a lock that outlives the bound names the holder-diagnosis step rather than the raw SQLite string. Fixes #1067.
