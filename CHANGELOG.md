...
### Added
- Skill content fingerprints in the outcome ledger (CocoIndex's memo-key idea applied to the ratchet): `outcome capture` and `outcome record` stamp each new record with the SHA-256 of the artifact's content (`content_fingerprint`), and `outcome rank` / `outcome explain` score the current-fingerprint cohort by default with lifetime counts still shown, so an edited skill earns its score back instead of inheriting signals for text that no longer exists. Pre-fingerprint records stay a distinct legacy cohort; the ledger remains append-only and the digest chain unchanged. Reconcile/promotion rules are untouched.
