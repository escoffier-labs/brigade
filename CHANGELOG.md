...
### Fixed
- `brigade care install` and `care status` on Windows now resolve the default `auto` backend to a printed Task Scheduler plan (`--backend schtasks`). Each printed `schtasks /Create` line uses that entry's own schedule, `/TR` through `C:\Windows\System32\cmd.exe`, an S4U principal (`/RU "%USERNAME%" /NP`; `/RL` omitted because LIMITED is the default and placing it between `/RU` and `/NP` makes Windows ignore `/NP`), and single-backslash workspace paths. `--json` returns the structured plan instead of plain text. `care status` reports whether each `BrigadeCare-*` task exists and when it last ran. (#1021)
