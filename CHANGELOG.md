...
### Docs
- Oracle browser compatibility is now explicit: Brigade still emits `oracle --engine browser -p <prompt>` without `--browser-cookie-sync`, Oracle 0.18.0 made cookie copy opt-in, hermetic tests do not prove live auth, and read-only applies to workspace writes only. Fixes #939.
