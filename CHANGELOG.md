...
### Fixed
- Work-inbox import publication now uses handle-relative no-follow opens on Windows (`CreateFileW` / `NtCreateFile` with `FILE_FLAG_OPEN_REPARSE_POINT`) so `memory care import-issues` and the rest of the import-inbox writers no longer raise `descriptor-relative import inbox operations are unavailable`. Junctions and other reparse points are still rejected, matching the POSIX `O_NOFOLLOW` walk. Directory component opens request `FILE_LIST_DIRECTORY|FILE_TRAVERSE` (plus `ADD_*|FILE_DELETE_CHILD` on mutation parents) and never `DELETE`, so a concurrent inbox-parent handle during the import-proof walk does not raise `STATUS_ACCESS_DENIED`. The import-proof authority store and HMAC key/sequence files shim the same no-follow flags, close the temp handle before `os.replace` (WinError 32), and fsync the parent through a backup-semantics directory handle. Windows native acceptance runs the import-inbox probe with the pipx-installed brigade interpreter, and runs `memory care scan` before `import-issues` so the refresh queue exists. It does not run extras-gated `center readiness import-issues`; that install leaves extras disabled. (#1022)
