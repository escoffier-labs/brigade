...
### Security
- Cached `evidence show` / MCP `show_evidence_bundle` now treat the entire same-UID-writable cache-ref as untrusted. Every free-form string on the filters / `code_reference` / collection projection is re-derived from an eligible ledger record or dropped unless it passes a hard bound and allowlist; unknown sibling keys are dropped. Completes the cache-ref echo class for #1031.
