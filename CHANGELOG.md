...
### Fixed
- Evidence-ledger read surfaces hide item bodies unless provenance parses and injection status is the validated typed value `clean`. Quarantined, pending, malformed, whitespace/case variants, and non-canonical digest representations stay metadata-only on search, MCP `search_evidence`, evidence bundles, Markdown export, and default `show`. (#1007, #1009)
