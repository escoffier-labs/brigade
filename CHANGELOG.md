...
### Fixed
- A fresh `init` / `operator quickstart` workspace no longer fails its own `doctor` or `security scan`. The workspace `AGENTS.md` template keeps the file-maintenance table in `memory/cards/` so the rendered file stays under the 12000-byte bootstrap budget on Windows CRLF, `RESTIC_PASSWORD_FILE` (and other `*_FILE` / `*_PATH` / `*_FILEPATH` assignments) are not plaintext-password findings, and the half-fed outcome-loop warning names `brigade work verify run --manifest` plus `verify/manifests/`. (#1025)
