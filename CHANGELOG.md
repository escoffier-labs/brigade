...
### Security
- Opt-in `authority_store.isolation = "external-key"` HMAC-signs new directory-authority bindings with a 0600 key stored outside the workspace and scanner-reachable tree (`$XDG_CONFIG_HOME/brigade/authority/store-hmac.key`, or `BRIGADE_AUTHORITY_KEY_FILE`). An existing HMAC envelope is always verified; flipping the flag off cannot skip a MAC check. A key override whose unresolved path sits under the workspace, including a directory-symlink prefix, a file symlink, or a `..` re-entry, is refused before any key bytes are written. `brigade doctor` WARNs when the flag is off. Fixes #957.
