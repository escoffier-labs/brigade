...
### Security
- Trust review is bound to an operator-minted capability. `brigade evidence trust review` generates a fresh in-memory HMAC secret, writes a `{item_id, from_digest, transition, nonce, expiry}` token to the engine on stdin, and never places that secret in the child env or argv. `miseledger trust review --mark-injection-clean` without a capability is refused for non-TTY callers, so a same-UID scanner cannot self-elevate. Direct engine invocation is unsupported. Fixes #1029.
