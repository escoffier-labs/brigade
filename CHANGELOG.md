...
### Fixed
- Work-inbox import publication now uses handle-relative no-follow opens on Windows (`CreateFileW` / `NtCreateFile` with `FILE_FLAG_OPEN_REPARSE_POINT`) so `memory care import-issues` and the rest of the import-inbox writers no longer raise `descriptor-relative import inbox operations are unavailable`. Junctions and other reparse points are still rejected, matching the POSIX `O_NOFOLLOW` walk. (#1022)
