...
### Security
- `work import promote --all` now reviews and promotes one inbox generation under the task-ledger lock. Matching rows are bound to that snapshot's identity and digest; a same-id swap or rewrite between review and promote is refused and does not create a task. Each item is all-or-nothing: the task ledger is written only after the late-window CAS succeeds, a refused promote persists no task, and an item reported as `failed:` is rolled back so it also persists no task. Fixes #1037.
