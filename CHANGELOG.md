...
### Fixed
- `handoff doctor`, `handoff lint`, and `ingest` now walk every configured `handoff-sources.json` root the way `handoff list` already did, so a pending file in a second root is counted, linted, and ingested instead of leaving those commands green on inboxes they never read. Fixes #1050.
