...
### Fixed
- Evidence crawl import (`miseledger import sourceharvest` / `crawl files|gitlog|docs`) retries SQLITE_BUSY from a concurrent receipt capture instead of failing the runbook, and a lock that outlives the bound names the holder-diagnosis step (process and database) rather than the raw SQLite string. Fixes #1067.
