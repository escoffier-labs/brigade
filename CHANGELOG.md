...
### Fixed
- Work-inbox import publication now uses handle-relative no-follow opens on Windows (`CreateFileW` / `NtCreateFile` with `FILE_FLAG_OPEN_REPARSE_POINT`) so `memory care import-issues` and the rest of the import-inbox writers no longer raise `descriptor-relative import inbox operations are unavailable`. Junctions and other reparse points are still rejected, matching the POSIX `O_NOFOLLOW` walk. The import-proof authority store shims the same no-follow flags, closes the temp handle before `os.replace` (WinError 32), and fsyncs the parent through a backup-semantics directory handle. (#1022)
