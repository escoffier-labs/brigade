...
- Evidence-ledger JSON/Markdown/MCP/HTTP exits now pass the complete serialized bundle through one last allowlist walker (`finalizeEvidenceResponse`). Ineligible or quarantined items collapse to `{id, eligibility_status, reason_code}`; `query`/`filters` and other unlisted keys are dropped, not trimmed. Cached `evidence show` / `show_evidence_bundle` regenerate live from item ids. A `kind=url` content hash can no longer authorize swapped artifact text. Fixes #1030, #1031, #1032.
