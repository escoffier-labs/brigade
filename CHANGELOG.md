...
### Fixed
- `brigade evidence crawl plan` now prints only miseledger commands that parse on the current CLI: positional `crawl memory <workspace>`, no `--root`/`--repo` flag forms, and no sourceharvest-backed `crawl files` / `crawl gitlog`. Omitted steps are stated. (#1024)
