...
### Security
- Every model-facing evidence projection — `materializeEvidenceBundle` (item_ids and no-item_ids), `evidence show` / MCP `show_evidence_bundle`, and `evidence list` — now goes through one sanitizer. Attacker-writable cache-ref fields (`query`, `filters.project` / `from` / `to`, collection name, unknown siblings) are dropped or allowlist-validated recursively; a length bound is not enough, so an under-bound planted string cannot survive. Completes the cache-ref echo class for #1031.
