...
### Added
- Fleet-sync Phase 5 adds deterministic, read-only hub export with `brigade fleet export --since <timestamp> --format jsonl|csv [--claims] [--db PATH] [--out PATH]`: events stream in primary-key order with fixed columns and their digest, claims stream by target without holder tokens, and repeated exports of unchanged data are byte-identical. The optional `[fleet.sink]` configuration remains disabled by default. When enabled, `brigade fleet sink` feeds deterministic CSV to `dolt table import -u` through a subprocess, with no Python dependency and no change to event reporting, claims, status, or dashboard paths. A missing Dolt binary is a documented no-op that prints the reason and exits 0. Fixes #1127.
