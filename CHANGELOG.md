...
### Added
- `brigade runs list --json`, `show --json`, and `latest --json` emit versioned `brigade.runs-list.v1` / `brigade.run-detail.v1` contracts, and `runs watch --json` records carry `brigade.run-watch.v1`. Show and latest share one allowlisted serializer; list summaries stay browser-safe (run id, status, bounded task, timestamps, duration, failure phase, mode, resume availability, optional parent run id) with `skipped_invalid` for unreadable directories. The allowlist excludes environment values, auth and control tokens, secret references, full prompts, transcript bodies, raw stdout/stderr, and absolute home paths. Child-run lineage from `runs child` is included when present, including recorded children discovered the same way as human `runs show`; legacy runs without lineage still serialize. Refs #631.
