...
### Fixed
- Provenance backfill now starts its write transaction with `BEGIN IMMEDIATE`, so a concurrent WAL commit cannot raise `SQLITE_BUSY_SNAPSHOT` (517). That error is not waited on by `busy_timeout`; contention becomes waitable `SQLITE_BUSY` (5) and the existing bounded retry still applies. Fixes #1083.
