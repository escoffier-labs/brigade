...
`grokbot_queue_claim` takes a required `job_id` and the optional `lease_id`, `lease_seconds`, and `worker_label`. When `lease_id` is omitted or sent as `null` the listener mints a uuid4 hex lease and returns it in the claim result as `lease_id`. Carry that value on `grokbot_queue_start`, `grokbot_queue_renew`, `grokbot_queue_complete`, `grokbot_queue_fail`, and `grokbot_queue_ack_cancel` for that job. A supplied `lease_id` is echoed back unchanged, and a malformed one is still refused with the generic validation error before any queue mutation. Worker identity and role remain deployment-fixed. Optional `worker_label` is a per-bot name such as `builder-2` that must match `[a-z0-9-]{1,32}`, so several Builder bots sharing one connector identity stay distinguishable. The listener validates it, forwards it to the hub unchanged, stores it on local file-backed job rows, and echoes it on the claim, status, renew, and complete projections and whenever a hub response carries it back. Hub-side storage is a pending fleet control-plane change: until that column lands a hub-authority queue accepts the label and returns nothing. Omit it or send `null` to leave the row unlabeled. A repeat claim from the lease that already holds the job replaces the stored label when it sends one, including on a job first claimed without a label, and leaves the stored label unchanged when it omits one.
...
If remote Grok Bot needs the listener, a Cloudflare Tunnel may publish only that listener endpoint. Put Cloudflare Access in front of the published endpoint. Do not publish the fleet hub or any local queue path, and do not replace Access with an unauthenticated tunnel policy. This guide intentionally does not include account-specific Tunnel or Access configuration. `pack doctor` for a pack that names a public hostname probes the local cloudflared metrics `/config` on loopback and fails with `ingress-port-mismatch` when that hostname's origin port is not the listener bind. It fails with `allowed-host-missing` when the running listener was not started with that Host. Those probes do not follow redirects and never print host-side paths or secrets. A connector with no public hostname and no public route is a local-only listener until a host is mapped; `pack doctor` reports that as `no-public-route: manual` unless `host_local` is true.
...
Under hub authority, preview and apply list completed Repository Scout report jobs from the hub with `include_all` using the operator or feed identity. Each listed job is paired with the local artifact and private task snapshot by `job_id`, and a snapshot whose `task_hash` disagrees with the hub job still fails closed as `snapshot-mismatch`. Multiple hub jobs that share a `task_hash` (scout retries with fresh idempotency keys) pair independently. Apply lists those hub jobs before acquiring the exclusive local queue lock so a slow hub cannot stall other local queue operations. Leftover pre-cutover rows in `jobs/` are not scanned. A hub job with no local artifact is counted as `unavailable` with reason `artifact-missing`. Under local authority, a completed local job with no stored report is still `unavailable` with reason `report-missing`. Those reasons appear in `--json` as `unavailable_reasons` and in text output when non-zero.
...
The supported batch conversion path is the CLI. Input schema `brigade.grokbot.live-findings.v1` is a regular owner-only mode-`0600` file with an `entries` array of at most 50 live normalized records. The command preflights every entry through `adapt_live_finding`, rejects duplicate identities, sorts by `producer`, `finding_id`, and `revision`, and writes schema `brigade.grokbot.findings.v1` atomically as mode `0600`. Any invalid later entry, unsafe permission, destination symlink, or symlink in any input or output parent component fails closed: the command does not read through the link and writes nothing. POSIX conversion walks every parent component with descriptor-relative `O_NOFOLLOW`. Windows private reads fail closed with `secure-owner-read-unavailable` until owner-SID/DACL enforcement exists; private writes remain fail-closed. Command output prints counts and identity handles only.
