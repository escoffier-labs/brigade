...
## Completion Detail Through v0.8.x

The sections below were moved verbatim from `ROADMAP.md` on 2026-06-09 when the roadmap was slimmed to direction-only. They preserve the full per-feature status bullets (implemented / strengthened / started) for everything built through v0.8.2. Command names here remain checkable against `docs/command-inventory.md`.
...
- Keep first-run setup usable for new operators: quickstart docs now show expected output, issue forms collect reproducible commands, setup labels are documented for triage, selected harnesses scope the watched handoff inboxes, and fresh installs get a local bootstrap handoff-ingest log so doctor output stays quiet until real handoff history exists. Status: strengthened.
- Make homegrown operator adoption measurable before migration: `brigade operator adopt plan/capture/import-issues` now inventories guidance files, harness roots, handoff inboxes, local state folders, and count-level scheduler/process surfaces; `brigade operator surfaces capture/list/doctor/review/reviews/import-issues` keeps a redacted external surface registry and ownership review receipts for shell crontab, OpenClaw cron, and PM2 coverage; and `brigade operator migration status/doctor/import-issues/consolidate` rolls adoption, surface reviews, pending imports, and pending tasks into one replacement-progress view while letting reviewed rollups supersede tiny record-level imports. These routes write local evidence and work imports without raw private details. Status: strengthened.
- Keep the real daily loop bounded and cheap: `brigade daily status` now uses a lightweight daily center snapshot, reports bounded section warnings instead of hanging, and the daily candidate list can route handoff ingest issues into `brigade handoff import-issues`. Status: implemented.
...
- Let repeated accepted plans and completed runs become draft workflow templates, repo rules, or skills through reviewed local proposals, without auto-installing or editing canonical memory. Status: strengthened with `brigade work plan-promote <id> --as template|rule|skill` for accepted plans and `brigade learn skill-candidates` / `brigade learn propose-skill <candidate-id>` for repeated learning evidence, both writing review proposals instead of installing or editing memory.
...
- Build a local tool catalog abstraction with source records, tool counts, search, describe, and call surfaces. Status: started with gitignored `.brigade/tools.toml`, read-only `brigade tools` discovery commands, and `brigade tools defaults` for merging current Brigade built-ins while preserving custom repo tools.
...
- Materialize reviewed harness projections from one local source of truth. Status: strengthened with explicit `brigade tools plan` and `brigade tools apply`, managed projection fingerprints, Codex-valid `SKILL.md` frontmatter rendering for skill projections, dry-run support, unmanaged or locally edited conflict protection, `operator sync-tools` default merging before apply, and `operator bootstrap-portable` for pack import, built-in source materialization, projection sync, and tool/skill health checks.
...
- Maintain a canonical local skill registry where each skill pack has `SKILL.md`, `skill.json` metadata, version, source, required tools, required MCP servers, supported harnesses, trust level, tests, and optional bundled assets or scripts. Status: strengthened with `brigade skills import`, local registry fingerprints, lint checks, changelog detection, trust score signals, and injection-signal warnings.
...
- Search and lint skill packs before use. Status: strengthened with `brigade skills search`, `brigade skills lint`, `brigade skills doctor`, and `brigade skills import-issues` for registry health and work-inbox routing.
- Keep the Skills Over MCP direction as an explicit experimental contract. Status: strengthened with `brigade skills serve-mcp`, which reports read-only resources such as `skill://registry/{skill_id}/SKILL.md`, `skill.json`, `CHANGELOG.md`, `compatibility.json`, and `history.json`, plus read-only tools such as `search_skills`, `get_skill`, `get_skill_metadata`, `get_skill_changelog`, `get_skill_compatibility`, `get_skill_history`, and `lint_skill`; `--stdio` serves the same read-only contract over local JSON-RPC without exposing install, publish, or fork tools.
...
- Add compatibility and version views for Codex, Claude Code, Gemini, OpenClaw, and MCP-native installs, including diffs, changelog, source, and trust score. Status: strengthened with `brigade skills compatibility`, `history`, and `diff`, including installed version, installed source/render fingerprints, version/source/render drift, changelog status, trust score, and JSONL install history.
...
- Build portable skill packs for reviewed cross-machine sharing. Status: started with `brigade skills pack build/list/show/archive/import`, which copies registry skill source folders into local reviewed packs and imports them into another workspace without installing them automatically.
...
- Aggregate local learning candidates without self-modification. Status: strengthened with `brigade learn plan/doctor/import-issues`, `brigade learn skill-candidates/propose-skill`, `brigade learn closeout/closeouts/closeout-show`, `brigade learn replay export/list/show/compare`, candidate routing into the scanner inbox, raw import text avoidance, accepted-risk or dismissal quieting, changed-fingerprint resurfacing, safe replay compare receipts, and reviewed skill proposal generation through the skill inbox.
...
- Scan Claude Code, Codex, OpenCode, Gemini, Hermes, OpenClaw, VS Code, Zed, dmux, and generic repo-local agent harness surfaces with explicit runtime-confidence labels. Status: started with cross-harness JSON wiring checks for `.brigade/`, `.claude/`, `.codex/`, `.opencode/`, `.openclaw/`, `.hermes/`, and Brigade templates, including path escapes, host-private paths, broad roots, private or insecure URLs, shell-like command fields, and `surface: session-chat` classification for exposed credentials in session or chat transcript paths.
...
- Export completed research runs into selected writer harness inboxes, including Codex, Claude Code, OpenCode, Hermes, and custom configured inboxes, as linted Memory Handoffs. Status: implemented with `brigade research export-handoff`, explicit `--inbox` or `--handoff-inbox` destinations, export fingerprints, lint checks, and no memory ingest, card edits, browser automation, daemon start, or remote mutation.
- Surface research handoff drift in existing review loops. Status: strengthened with research show/list export status, `brigade research handoffs doctor`, `brigade research handoffs import-issues`, work brief health, center review items, and release readiness evidence for missing, missing-path, and stale exports while keeping research handoffs in the normal handoff draft and work inbox paths.
