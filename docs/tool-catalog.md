...
supported_harnesses = ["claude", "codex", "opencode", "antigravity"]
...
Supported harness labels are local conventions. Brigade recognizes Claude Code, Codex, OpenCode, Antigravity, Hermes, OpenClaw, MCP, and scripts through the labels `claude`, `codex`, `opencode`, `antigravity`, `hermes`, `openclaw`, `mcp`, and `scripts`.
