...
Receipts include call id, tool id, family, status, timestamps, duration, exit code, timeout status, command label, cwd, redacted args and rendered arguments, redacted stdout/stderr summaries, stdout/stderr log paths, contract/source/call/approval fingerprints, approval metadata, permissions, effects, runtime snapshot, policy decision, env labels used, and projection summary. When either stored summary is compacted, the additive `output_compaction` object records the deterministic reducer id, SHA-256 rule hash of the declared reducer contract, total omitted character count, per-stream omitted counts, and whether the raw stdout/stderr log files were present when the receipt was written. Omitted counts are reducer omissions measured on the post-redaction text after whitespace normalization and before the stored summary is written. Whitespace-only normalization can still produce a nonzero omitted count. The rule hash fingerprints the declared reducer contract (secret-pattern redaction, whitespace collapse, character limit, trailing trim before the ellipsis suffix, and truncation suffix) rather than self-hashing implementation details. A behavior change must bump `reducer_id` or change `rule_hash`. Readers must not treat the same fingerprint as proof of unchanged reducer semantics. `raw_retrievable` is evaluated at receipt write time and is not re-evaluated after pruning. Uncompacted stored receipts omit `output_compaction`, while public run summaries expose `output_compaction` as null. Existing receipt readers remain compatible. MCP receipts also include server id, tool name, request id, redacted request payload, redacted response summary, and MCP response count. Raw stdout and stderr logs stay local and gitignored.
...
Release readiness and release candidate evidence treat tool packs and sync plans as review evidence. The evidence records the latest pack, whether the pack is stale or missing, the parity closeout state, sync-plan blocker counts, approval queue counts, run history counts, and checkpoint counts. Release commands never apply projections or run tool calls.
