...
Status: spec, awaiting green-light. Not implemented. Sibling to
`context-blind-spot.md`: a third cohort axis on the outcome ledger.
...
### Phase 1: record and surface, no scoring change

Stamp the `route` manifest and `route_fingerprint` onto new outcome records.
Surface a routed-vs-unrouted breakdown in `outcome explain`. The default score,
