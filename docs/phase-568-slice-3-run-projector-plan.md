...
- [x] Confirm the fixture carries every preserved field and nothing outside the ownership map:
...
Observed: 22 passed, 1 failed. The one failure was `Failed: missing golden expected fixture: .../golden-projection.expected.json` from `test_golden_replay_matches_expected_bytes`.
...
- [x] Generate `tests/fixtures/run-lifecycle/golden-projection.expected.json`:
...
- [x] Hand-review the generated bytes against this checklist. The review is the point of the golden convention, so do not skip it:
...
Observed: `23 passed` (20 test functions, one parametrized over four cases).
