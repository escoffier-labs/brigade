...
Set `authority_store.isolation = "external-key"` under `[authority_store]` to HMAC-sign **new** bindings with an operator-provisioned key that lives **outside** the workspace and the scanner-reachable `.brigade` tree. An existing HMAC envelope is always verified; the flag never skips a MAC check. The first signed write also records a user-level sticky marker at `~/.brigade/authority-signed/<target-fingerprint>` (override: `BRIGADE_USER_DIR`), and observing the enabled flag itself records the isolation posture marker at `~/.brigade/authority-isolation/<target-fingerprint>`. A raw unsigned record for a marked target fails closed even if the repo flag is off. Downgrade is `brigade security authority downgrade --target <workspace> --confirm` only: it writes the inner unsigned record, removes both sticky markers, and sets isolation off so the next read cannot recreate the marker. `brigade doctor` WARNs when the flag is off.
