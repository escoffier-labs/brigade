...
Set `authority_store.isolation = "external-key"` under `[authority_store]` to HMAC-sign **new** bindings with an operator-provisioned key that lives **outside** the workspace and the scanner-reachable `.brigade` tree. An existing HMAC envelope is always verified; the flag never skips a MAC check. The first signed write also records a user-level sticky marker at `~/.brigade/authority-signed/<target-fingerprint>` (override: `BRIGADE_USER_DIR`), and observing the enabled flag itself records the isolation posture marker at `~/.brigade/authority-isolation/<target-fingerprint>`, bound to the workspace's stable root identity (device/inode — the same identity the signed record carries). Observation is fail-closed: if the posture marker cannot be persisted, observing `external-key` refuses to proceed instead of warning. A raw unsigned record for a marked target fails closed even if the repo flag is off. Downgrade is `brigade security authority downgrade --target <workspace> --confirm` only: it writes the inner unsigned record, removes both sticky markers, and sets isolation off so the next read cannot recreate the marker. `brigade doctor` WARNs when the flag is off.
...
- **Canonical dedupe suppression follows the workspace posture.** In a workspace with `authority_store.isolation = "external-key"`, suppressing an incoming duplicate requires that same verified snapshot plus the bound persisted sidecar. In a workspace without it, no verifier-signed store can exist, and a same-uid writer can already rewrite the import inbox directly (issue #1093 tracks that same-uid boundary), so suppression falls back to the pre-#881 evidence grade: a persisted sidecar proof validated against the workspace's own authority record (the `_unsigned_dedupe_proof_for_non_isolated_workspace` predicate; receipt bytes are not revalidated on this fallback). The first fallback selection per process logs one warning naming the downgrade.
- **The isolation selector is not trusted alone.** `.brigade/security.toml` is repo-writable, so a same-uid scanner could flip `authority_store.isolation` back to `"off"` or corrupt the file (invalid config normalizes to off) to select the unsigned fallback. The first time genuine `external-key` isolation is observed, Brigade records a per-target isolation posture marker under the operator's user-level directory at `~/.brigade/authority-isolation/<target-fingerprint>` (override: `BRIGADE_USER_DIR`), outside every workspace beside the other per-target authority state. Marker reads are tri-state (#881): `present`, `confirmed-absent`, or `unknown` — only a positively read healthy marker store that names no governing marker reports absence; an unreadable root, unreadable or malformed entries, non-regular entries, and incomparable identity bindings all report unknown, which fail-closed callers treat like present. The marker is bound to the workspace's stable root device/inode identity (the same identity the signed authority record carries), so it keeps governing a renamed workspace — the authenticated reanchor transfers and re-verifies the posture onto the new path before any unsigned state is evaluated — while an unrelated workspace that recycles the old path is never blocked by the stale path-keyed marker. The unsigned dedupe fallback is admitted only when the marker status is positively confirmed absent, the configuration reads healthy `off`, and observing isolation never persisted a failure; every other combination refuses it. Clearing the marker requires the explicit, audited `brigade security authority downgrade --target <workspace> --confirm` command, which removes both sticky markers, unwraps the signed store when possible, and sets isolation off.
