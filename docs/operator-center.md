...
brigade work phases session checkpoints archive <checkpoint-id|latest>
brigade work phases session recovery-note <session-id|latest>
brigade work phases session recovery-notes list
brigade work phases session recovery-notes show <note-id|latest>
brigade work phases session recovery-notes closeout <note-id|latest>
brigade work phases session risk <session-id|latest>
brigade work phases session verification <session-id|latest>
brigade work phases session privacy <session-id|latest>
...
Phase execution sessions group a declared AFK range into one local record under `.brigade/work/phases/sessions/`. `session start` records the range, current phase, status summary, commit and test counts, report references, closeout state, and next command. `session next` classifies the safest next step across missing records, pending phases, stale in-progress work, unverified phases, missing commit or push evidence, unreviewed pushed phases, and session closeout. `session risk` summarizes next-step blockers, checkpoint drift, open recovery notes, and phase doctor issues in one read-only view. `session verification` rolls up expected, passed, failed, skipped, and deferred verification across the session range. `session privacy` rolls up clean, blocked, and missing privacy checks across the session range without running scans. `session handoffs` rolls up linted, drafted, failed, deferred, and missing handoff coverage without writing a draft. `session next` and `session resume` also surface the latest checkpoint summary and issue count when recovery metadata exists. `session resume` records that recommendation as local metadata without executing the suggested command. `session checkpoint` records a local recovery point with safe summary, notes, current next-step state, and suggested command without executing anything. `session checkpoints list/show/compare` inspect those local recovery points and detect stale saved next-step state without executing suggested commands. `session checkpoints import-issues` turns blocked or stale checkpoint issues into normal deduped work inbox tasks without promotion or execution. `session checkpoints archive` moves one checkpoint into local JSONL archive metadata and removes the active checkpoint file. `session recovery-note` records safe resume context and evidence labels as local metadata, while `session recovery-notes list/show/closeout` make those notes reviewable, closeable, and visible in session activity. Daily planning can surface checkpoint issues as `phase-session-checkpoint` candidates that suggest checkpoint import commands, and daily run can write one local session checkpoint as its single bounded action. `session list`, `show`, and `closeout` only read or update local session metadata.
...
Release and operator surfaces include compact phase session state. Work brief, work doctor, center status, center reviews, release doctor, release candidate evidence, and candidate compare can show active sessions, missing session reports, unresolved session actions, blocked or stale session checkpoints, and newer session evidence after a candidate was built. Work brief includes the latest checkpoint and compare summary in its phase ledger block. Release candidate bundles preserve the latest checkpoint and compare summary as local evidence. Center reviews include blocked or stale phase-session checkpoint items with suggested inspect or compare commands. Phase action planning can also turn blocked or stale checkpoint issues into normal local phase actions without executing the suggested command. Session report bundles include a recovery section with latest checkpoint, checkpoint compare issue count, and recovery-note summaries.
...
`brigade repos adoption` scans enabled `.brigade/repos.toml` entries and reports harness wiring separately from recent use. Repeat `--harness` to select Claude or Cursor and set the observation window with `--days`. JSON rows use safe repo ids and labels, stable `<repo-id>:<harness>` keys, one of seven adoption states, wiring dimensions, aggregate session evidence, and one read-only next command. The monitor block identifies current `unwired`, `stale`, and `bypassed` rows and provides a fingerprint that a scheduled caller can compare with its prior result.
