...
- [MiseLedger](https://github.com/escoffier-labs/miseledger): local-first evidence ledger. One binary crawls sessions, files, git history, and chat sources (`miseledger crawl ...`), normalizes to `miseledger.adapter.v1` JSONL, stores them in SQLite with FTS5, and emits Brigade-ready evidence bundles. There is no separate exporter install.
- Historical note only: StationTrail and SourceHarvest were absorbed into MiseLedger crawl in v0.3.0. Their archived repos ([stationtrail](https://github.com/escoffier-labs/stationtrail), [sourceharvest](https://github.com/escoffier-labs/sourceharvest)) are migration notes, not active products.
