...
It is meant for people running real tools, real docs, and real automation across OpenClaw, Claude Code, Codex, OpenCode, Antigravity, Hermes, or a similar harness.
...
|---|---|
| **harness** | An AI agent program: Claude Code, Codex, OpenCode, Antigravity, OpenClaw, Hermes. |
...
- Multi-harness handoffs: `.claude/memory-handoffs/`, `.codex/memory-handoffs/`, `.opencode/memory-handoffs/`, `.antigravity/memory-handoffs/`, source coverage checks, linting, reconciliation receipts, issue imports, sync repair, and archive closeouts.
...
- Shared skills: reviewed `SKILL.md` packs with metadata, provenance, linting, fingerprints, trust score, changelog, install history, diffs, portable packs, publish proposals, and one-command installation across Codex, Claude, OpenCode, Antigravity, OpenClaw, Hermes, and MCP-resource targets.
...
`brigade handoff doctor` reports pending `.claude/memory-handoffs/`, `.codex/memory-handoffs/`, `.opencode/memory-handoffs/`, and `.antigravity/memory-handoffs/` files that are not covered by that local source list.
...
`codex`, `claude`, `opencode`, `antigravity`, and `ollama:<model>`. Brigade shells out to those tools and keeps no provider keys. The Antigravity adapter uses the installed `agy --print` non-interactive CLI path.
...
Use `--agent-cli claude`, `--agent-cli opencode`, `--agent-cli antigravity`, or `--agent-cli ollama:<model>` to run dogfood through another installed CLI. Writer CLIs with known inboxes, such as Claude Code, OpenCode, and Antigravity, default handoffs to their own memory-handoff folders when selected.
...
Exporting the handoff is explicit. `brigade research export-handoff <run-id> --inbox codex` copies the completed run's linted handoff into a selected writer inbox such as Codex, Claude Code, OpenCode, Antigravity, or Hermes. Use `--handoff-inbox <path>` for a custom writer. Brigade records the export fingerprint on the run and surfaces missing, stale, or missing-path exports in `research show`, `work brief`, `center reviews`, and release readiness evidence. `brigade research handoffs doctor` gives a focused export-health check, and `brigade research handoffs import-issues` routes export repairs into the normal work inbox with stable source fingerprints.
...
- `brigade operator sync-tools` projects tracked `tools/*.md` sources into local Claude, Codex, OpenCode, and Antigravity harness folders.
...
- `brigade handoff list` lists local Memory Handoff drafts from `.claude/memory-handoffs/`, `.codex/memory-handoffs/`, `.opencode/memory-handoffs/`, `.antigravity/memory-handoffs/`, and configured source inboxes.
...
- `brigade tools init` writes gitignored `.brigade/tools.toml` with local `simplify`, `superpowers`, `frontend`, and `antislop` examples projected across Claude, Codex, OpenCode, Antigravity, Hermes, OpenClaw, MCP Markdown resources, and script Markdown surfaces.
...
- `brigade skills install security-review --target codex`, `--target claude`, `--target opencode`, `--target antigravity`, `--target openclaw`, `--target hermes`, or `--target mcp` materializes one reviewed skill into a specific harness shape.
- `brigade skills install security-review --target all` installs the same reviewed skill into Codex, Claude, OpenCode, Antigravity `.antigravity/skills`, OpenClaw, Hermes, and MCP-resource folders, writing per-harness receipts. Built-in adapters normalize target-specific output, for example adding Codex `SKILL.md` frontmatter when the portable source does not already have it.
...
- `brigade skills adapters list --include-planned` shows built-in and planned harness adapters, including Antigravity as a built-in target and Pi or Cursor as planned adapter targets.
...
Harness support is intended to stay adapter-based. The current built-ins cover Codex, Claude, OpenCode, Antigravity, OpenClaw, Hermes, and MCP resources, and future adapters can add Pi, Cursor, or similar agent surfaces without changing the skill registry contract.
...
- Use `--handoff-inbox <path>` for Codex, OpenCode, Antigravity, Hermes, OpenClaw, or another writer inbox.
...
- Harness wiring checks cover repo-local agent JSON across `.brigade/`, `.claude/`, `.codex/`, `.opencode/`, `.antigravity/`, `.openclaw/`, `.hermes/`, and Brigade templates, including path escapes, host-private paths, insecure or private URLs, and shell-like command fields.
