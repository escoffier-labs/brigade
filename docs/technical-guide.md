...
The fingerprint sees the artifact's own files, not the runtime harness around it, the same caveat CocoIndex documents for undecorated helpers. It also does not yet walk a card's `[[wiki-links]]` transitively, so editing a linked card does not invalidate a card that references it; that transitive tracking is future work.
