...
Use `brigade run cloud register` / `adopt` to record dispatches (Codex Cloud register-on-dispatch is automatic from `codex-cloud` seats; Cursor cloud remains an unwired source until an API key exists). `brigade run cloud status --json` joins the registry against best-effort provider state and GitHub branch/PR ground truth. `brigade run cloud sweep` writes a receipted report only — nothing is deleted automatically. Stale READY entries (threshold `stale_ready_hours`, default 6) surface in `brigade work brief`.
