...
Those release and operator surfaces also include latest phase session and session report references, warning when active sessions are stale, unreported, or changed after a candidate was built. Release doctor also warns when the latest active phase session checkpoint is blocked or no longer matches the current session next-step state.
