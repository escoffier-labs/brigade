...
Check handoff pipeline health, read the daily driver snapshot, and build the local operator report bundle.
...
Run an approved maintenance runbook. Pin the steps you trust, restrict `allowed_commands` to `brigade`, and keep the JSON under `.brigade/runbooks/`.
