...
<svg id="map-graph" role="group" aria-labelledby="map-graph-heading map-graph-description" viewBox="0 0 960 640" tabindex="0">
...
(() => {
  ...
  const show = (node) => {
    ...
    command.textContent = `graphtrail map --out graphtrail-map.html -- ${shellQuote(node.file_path)}`;
  ...
  const renderGraph = () => {
    ...
    for (const edge of edges) {
      ...
      const sourcePosition = positions.get(edge.source);
      const targetPosition = positions.get(edge.target);
      const sourceRadius = nodeRadius(sourceNode) + 2;
      const targetRadius = nodeRadius(targetNode) + 2;
      const edgeDx = targetPosition.x - sourcePosition.x;
      const edgeDy = targetPosition.y - sourcePosition.y;
      ...
      line.setAttribute('x1', String(sourcePosition.x + (edgeDx / edgeDistance) * sourceRadius));
      line.setAttribute('y1', String(sourcePosition.y + (edgeDy / edgeDistance) * sourceRadius));
      line.setAttribute('x2', String(targetPosition.x - (edgeDx / edgeDistance) * targetRadius));
      line.setAttribute('y2', String(targetPosition.y - (edgeDy / edgeDistance) * targetRadius));
