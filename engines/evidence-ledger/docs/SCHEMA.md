...
Python consumers load `brigade.trust-policy.v1` and omit `unknown` / `quarantined` from default briefs, context, cite, and promote. `miseledger trust review` is the operator/verifier write path: it requires the current envelope digest and appends one `provenance_events` row. Moving the trust label does not change injection status. `--mark-injection-clean` is the explicit operator decision that records an injection-status transition to `clean` with a separate audit event. Envelope `verified` admits an indexed receipt to consumers but does not make it scoreable.
