...
### Security
- Every model-facing evidence projection — `materializeEvidenceBundle` (item_ids and no-item_ids), `evidence show` / MCP `show_evidence_bundle`, and `evidence list` — goes through one sanitizer. Attacker-writable cache-ref fields (`query`, `filters.project` / `from` / `to`, collection name, unknown siblings) are dropped or allowlist-validated recursively; a length bound is not enough.
