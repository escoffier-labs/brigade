...
- Round 3 (#1201): stationtrail resolution is now singular across probes, approval hashing, dry-runs, and imports: the executable is resolved once, opened, hashed through the opened descriptor, and every subsequent exec runs that same resolved absolute path after re-verifying (device/inode on unix; size/mtime elsewhere) that the artifact under the resolved path is unchanged since hashing. A swapped PATH entry or replaced binary between hash and exec is refused instead of executed. Residual: an in-place rewrite preserving device, inode, size, and mtime between verification and exec is not detected — install stationtrail in a root-owned directory (documented).
