...
### Security
- Cached `evidence show` / MCP `show_evidence_bundle` treat the entire same-UID-writable cache-ref as untrusted. Filters, `code_reference`, and collection free-form strings are re-derived from eligible ledger records or dropped unless they pass a hard bound and allowlist; unknown sibling keys, including unexpected top-level cache-ref fields, are dropped at the materialize choke point.
