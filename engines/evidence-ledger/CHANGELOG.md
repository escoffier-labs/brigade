...
### Security
- Ineligible / quarantined cached evidence drops the entire cache-ref-derived envelope from model-facing `evidence show`, MCP `show_evidence_bundle`, `evidence list`, and materialize (including when `item_ids` is omitted). Eligible items keep their live reconstruction. A byte bound is not the injection control. Residual of #1031.
