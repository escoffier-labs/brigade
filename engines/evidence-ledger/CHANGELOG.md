...
### Security
- Evidence CLI, MCP, and HTTP exits now sanitize the complete serialized payload through one last allowlist walker. Ineligible items become `{id, eligibility_status, reason_code}`; request `query`/`filters` are not reflected; cached show regenerates live; a URL content hash cannot authorize swapped artifact text. Fixes #1030, #1031, #1032.
