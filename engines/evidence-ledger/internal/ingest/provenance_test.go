...
func TestReviewTrustRefusesTamperedIncomingEnvelope(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			want := provenance.ContentSHA256(text)
			err = ReviewTrust(db, itemID, want, "reviewed", "operator:brigade evidence trust review", map[string]any{"kind": "operator-review"}, TrustReviewOpts{MarkInjectionClean: true})
			if err == nil {
