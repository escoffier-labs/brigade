...
func openDataDirFile(dirPath, name string) (*os.File, error) {
	...
	if err != nil {
		if errors.Is(err, syscall.ELOOP) || errors.Is(err, syscall.ENOTDIR) {
			return nil, fmt.Errorf("data directory %s refused (symlink or not a directory): %w", dirPath, err)
		}
		return nil, fmt.Errorf("data directory %s could not be opened: %w", dirPath, err)
	}
	var st syscall.Stat_t
	if err := syscall.Fstat(dfd, &st); err != nil {
		_ = syscall.Close(dfd)
		return nil, fmt.Errorf("data directory %s could not be stated: %w", dirPath, err)
	}
	if st.Mode&syscall.S_IFMT != syscall.S_IFDIR {
		_ = syscall.Close(dfd)
		return nil, fmt.Errorf("data directory %s is not a directory", dirPath)
	}
	if uid := os.Getuid(); st.Uid != uint32(uid) {
		_ = syscall.Close(dfd)
		return nil, fmt.Errorf("data directory %s owned by uid %d, running uid %d", dirPath, st.Uid, uid)
	}
