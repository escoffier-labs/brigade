...
def _import_search_runner(calls, *, export_lines="{}\n"):
    """Fake runner for smoke_import_and_search that records every argv."""
    ...
    def runner(argv, **kwargs):
        ...
        if name == "brigade" and "export" in argv:
            out = Path(argv[argv.index("--out") + 1])
            out.write_text(export_lines, encoding="utf-8")
            return subprocess.CompletedProcess(argv, 0, "ok", "")
        if name == "miseledger" and "import" in argv:
            return subprocess.CompletedProcess(argv, 0, '{"inserted_items": 1, "already_known": false}', "")
        if name == "miseledger" and "sql" in argv:
        ...
        if name == "miseledger" and "search" in argv:
            return subprocess.CompletedProcess(
                argv,
                0,
                json.dumps({"query": "marker", "results": [{"id": "73a4dea0caf74b08af5e0f59", "snippet": ""}]}),
                "",
            )
        return subprocess.CompletedProcess(argv, 0, "ok", "")
...
def _import_search_binaries(tmp_path):
    brigade = tmp_path / "brigade"
    brigade.write_text("#!/bin/sh\nexit 0\n")
    brigade.chmod(0o755)
    miseledger = tmp_path / "miseledger"
    miseledger.write_text("#!/bin/sh\nexit 0\n")
    miseledger.chmod(0o755)
    return brigade, miseledger


def test_smoke_import_and_search_leaves_a_receipt_the_capture_cursor_has_not_consumed(acceptance_module, tmp_path):
    """`--capture` indexes and advances the export cursor, so `--new-only` needs a fresh receipt.

    ``work verify run --capture`` calls
    ``work_cmd.verification._attach_miseledger_indexing``, which exports and
    imports every pending receipt and advances
    ``.brigade/work/miseledger-export-cursor.json``. Exporting with
    ``--new-only`` straight afterwards writes an empty file and the import
    reports ``inserted_items=0``. A second, uncaptured ``--no-reuse`` run is what
    leaves a receipt behind the cursor, matching the Windows acceptance flow.
    """
    calls = []
    brigade, miseledger = _import_search_binaries(tmp_path)

    acceptance_module.smoke_import_and_search(
        brigade,
        miseledger,
        tmp_path / "work",
        runner=_import_search_runner(calls),
        env={"HOME": str(tmp_path / "profile")},
    )

    verify_runs = [argv for argv in calls if argv[1:4] == ["work", "verify", "run"]]
    assert len(verify_runs) == 2
    captured, uncaptured = verify_runs
    assert "--capture" in captured
    assert "--no-reuse" not in captured
    # The export cursor only ignores a receipt that capture never indexed, and
    # --no-reuse is what stops the identical planned command reusing the
    # captured run instead of producing a new receipt.
    assert "--capture" not in uncaptured
    assert "--no-reuse" in uncaptured

    export = next(argv for argv in calls if "export" in argv and "--new-only" in argv)
    assert calls.index(uncaptured) < calls.index(export)


def test_smoke_import_and_search_rejects_an_empty_new_only_export(acceptance_module, tmp_path):
    """An empty --new-only export means the cursor already consumed every receipt."""
    calls = []
    brigade, miseledger = _import_search_binaries(tmp_path)

    with pytest.raises(acceptance_module.AcceptanceError, match="wrote no receipts"):
        acceptance_module.smoke_import_and_search(
            brigade,
            miseledger,
            tmp_path / "work",
            runner=_import_search_runner(calls, export_lines=""),
            env={"HOME": str(tmp_path / "profile")},
        )
