...
class TestInventory:
    ...
    def test_sanitize_session_never_carries_activity_kinds(self):
        ...
        # never carries it, so a caller that reads session["activity_kinds"] is
        # always reading None (#1419 review).
        session = jules_cloud.sanitize_session(
            {
                "id": "sess-plan",
                "state": "COMPLETED",
                "activities": [{"planGenerated": {"title": "PRIVATE-PLAN"}}],
                "activityKinds": ["planGenerated"],
            }
        )
        assert session is not None
        assert "activity_kinds" not in session
        assert set(session) == {
            "id",
            "state",
            "create_time",
    ...
    def test_list_activities_exposes_plan_evidence_for_the_tracker(self):
        # The kinds the tracker classifies on must survive a real paginated read.
        opener = FakeOpener(
            [
                _json_response(
                    {
                        "activities": [
                            {"id": "act-1", "userMessaged": {"message": "PRIVATE"}},
                            {"id": "act-2", "planGenerated": {"title": "PRIVATE-PLAN"}},
                            {"id": "act-3", "sessionCompleted": {}},
                        ],
                        "nextPageToken": None,
                    }
                ),
        ...
        activities = jules_cloud.list_activities("sess-1", _FAKE_KEY, opener=opener.open, max_pages=1, max_items=30)
        assert [row["kind"] for row in activities] == ["userMessaged", "planGenerated", "sessionCompleted"]
        assert "PRIVATE" not in json.dumps(activities)
