...
def test_builtin_stations_present():
    ...
    assert {"core", "skills", "memory", "guard", "security"} <= names
...
def test_resolve_by_name_and_alias():
    ...
    assert registry.resolve("skillet").name == "skills"
...
def test_stations_declare_attached_tools():
    ...
    assert tokens is not None and set(tokens.tools) == {"token-glace"}
...
def test_evidence_station_declares_miseledger():
    evidence = registry.resolve("evidence")
    assert evidence is not None
    assert set(evidence.tools) == {"miseledger"}
