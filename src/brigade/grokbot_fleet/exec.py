"""Bounded Fleet Steward subprocess runner."""
...
from .runtime_config import fleet_probe_environment
...
_CHILD_FAILURES: WeakKeyDictionary[FleetError, tuple[int, str, str]] = WeakKeyDictionary()
...
class ExecRequest:
    ...
    accept_numeric_exit: bool = False
...
def child_failure_output(error: FleetError) -> tuple[int, str, str] | None:
...
def run_exec(request: ExecRequest, *, runner: Runner | None = None) -> PrivateExecResult:
    if not _bounded_int(request.timeout_ms, EXEC_MIN_TIMEOUT_MS, EXEC_MAX_TIMEOUT_MS):
        raise FleetError("invalid_request", "Fleet execution request is invalid")
    ...
    prepared = ExecRequest(
        ...
        env=fleet_probe_environment(request.env),
        stdin=request.stdin,
        accept_numeric_exit=request.accept_numeric_exit,
    )
    try:
        return (runner or _subprocess_runner)(prepared)
    except ChildTimeout:
        raise FleetError("timeout", "Fleet observation timed out") from None
    except ChildOversize:
        raise FleetError("protocol_error", "Fleet observation was invalid") from None
    except ChildFailure as exc:
        error = FleetError("unavailable", "Fleet observation is unavailable")
        ...
        raise FleetError("unavailable", "Fleet observation is unavailable") from None
...
def _subprocess_runner(request: ExecRequest) -> PrivateExecResult:
    ...
    if process.returncode:
        if request.accept_numeric_exit and isinstance(process.returncode, int):
