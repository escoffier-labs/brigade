...
_CLAIM_DESCRIPTION = (
    ...
    "({lease_default} seconds here). A supplied worker_label names this bot on the job row: the "
    "listener validates it, forwards it to the hub unchanged, stores it on local job rows, and "
    "echoes it on claim, status, renew, and complete. Hub-side storage is a pending fleet "
    "control-plane change, so against a hub-authority queue the label is accepted and comes back "
    "absent until that column lands. Sending worker_label on a repeat claim of a job this lease "
    "already holds replaces the stored label, including on a job first claimed without one; "
    "omitting it leaves the stored label unchanged. The result carries the granted "
