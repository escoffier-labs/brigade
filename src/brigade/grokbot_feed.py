...
def worker_instructions(*, header: str, issue_number: int, base_ref: str, verification_commands: list[str]) -> str:
    ...
    return (
        ...
        "is in. Stop as soon as the pull request is open and the job is completed with its URL. The time cap is "
        "the latest you may still be running, not a target; never wait for CI, review, merge, or a timer.\n\n"
