    ...
    for trust, title, note in groups:
        ...
        for f in rows:
            ...
            source_html = (
                ...
                if f.source.startswith(("http://", "https://"))
                else f"<code>{source}</code>"
            )
            parts.append(
                f'<div class="src {trust}"><div class="tag">{trust}</div>'
...
def render_html(*, question: str, markdown_report: str, findings: List[Finding], stats: Dict[str, Any]) -> str:
