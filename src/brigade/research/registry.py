...
def create_run(
    target: Path, *, question: str, run_id: str, caps: Dict[str, Any], manifest: Optional[Dict[str, Any]] = None
) -> str:
...
def finish_run(
    ...
    *,
    ...
    _update(
        target, run_id, status=status, stats=stats, artifacts=artifacts, blockers=blockers or [], completed_at=_now()
    )
