...
def render_unit(config: dict[str, Any], *, python: str, exec_root: Path) -> str:
    ...
    hub_environment = ""
    if hub_token_file is not None:
        env_name = f"BRIGADE_GROKBOT_{instance.replace('-', '_').upper()}_HUB_TOKEN_FILE"
...
def _validate_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "env" and set(reference) == {"kind", "name"} and isinstance(reference.get("name"), str):
        if grokbot_mcp.ENVIRONMENT_NAME_RE.fullmatch(reference["name"]):
            ...
            raise grokbot_mcp.ConfigurationError("invalid")
    elif kind == "file" and set(reference) == {"kind", "path"} and isinstance(reference.get("path"), str):
        _systemd_quote(reference["path"])
    else:
        raise grokbot_mcp.ConfigurationError("invalid")
    if "hub_token_file" in config:
        raw_hub_token_file = config["hub_token_file"]
        if not isinstance(raw_hub_token_file, str):
            raise grokbot_mcp.ConfigurationError("invalid")
        config = dict(config)
        config["hub_token_file"] = _validated_hub_token_file(Path(raw_hub_token_file))
    return config


def _validated_hub_token_file(path: Path) -> str:
    """Accept one existing mode-0600 absolute regular token-file reference."""
    try:
        expanded = path.expanduser()
        if not expanded.is_absolute():
            raise OSError("hub token path is not absolute")
        resolved = expanded.resolve(strict=True)
        info = resolved.stat()
        if not stat.S_ISREG(info.st_mode) or stat.S_IMODE(info.st_mode) & 0o077:
            raise OSError("hub token file is not protected")
        _systemd_quote(str(resolved))
