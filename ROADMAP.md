...
- `brigade work phases` provides a gitignored execution ledger for long unattended multi-phase work, including no-silent-compression checks, completion evidence requirements, range status, review closeouts, compare checks, reports, inbox issue routing, and release evidence. Status: strengthened through phase 202 compare checks.
