...
- `brigade work phases` provides a gitignored execution ledger for long unattended multi-phase work, including no-silent-compression checks, completion evidence requirements, range status, review closeouts, compare checks, commit reconciliation, privacy scans, handoff drafts, action queues, evidence attachments, verification matrices, daily-driver candidates, report closeouts, report compare checks, AFK execution sessions, session next/resume recovery, session activity timelines, session reports, daily session candidates, inbox issue routing, and release/operator evidence. Status: strengthened through phase 221 session activity.
