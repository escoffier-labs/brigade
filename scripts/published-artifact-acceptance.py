...
def smoke_import_and_search(
    ...
    *,
    ...
    # `--capture` runs work_cmd.verification._attach_miseledger_indexing, which
    # exports and imports every pending receipt and advances
    # .brigade/work/miseledger-export-cursor.json. The captured run above is
    # therefore already behind the cursor, and a `--new-only` export taken now
    # writes an empty file whose import reports inserted_items=0. Leave a second
    # receipt the cursor has not consumed, with `--no-reuse` so the identical
    # planned command produces a new run instead of reusing the captured one.
    # The Windows acceptance flow orders these two runs the same way.
    run_checked(
        [
            managed_brigade,
            "work",
    ...
    if export_path.stat().st_size == 0:
        raise AcceptanceError(
            f"receipts export miseledger --new-only wrote no receipts to {export_path}; "
            "the export cursor already consumed every local receipt"
        )
